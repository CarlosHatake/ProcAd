    ...
    Public Sub pantallaInicio()
        With Me
            Try
                ...
                ConexionBD.Close()
                ._txtFolio.Text = dsSolicitud.Tables(0).Rows(0).Item("id_ms_anticipo_proveedor").ToString()
                ...
                ElseIf ._txtTipoAnt.Text = "3" Then
                    ...
                    '.lbl_CFDIAnticipoAgenteAduanal.Visible = False
                    .btnAceptarAgenteAduanal.Visible = False
            ...
            Try
                ...
                If .gvAdjuntosEscenario1.Rows.Count() = 0 Then
                    ...
                    Else
                        ban = 1
                    End If
                    .litError.Text = .litError.Text + "Elija al menos un archivo"
                End If
                ...
                If ban = 0 Then
                    ...
                    query = "EXEC SP_I_ms_comprobacion_anticipo @id_ms_anticipo_proveedor, @id_usr_solicita, @fecha_solicita, @tipo_escenario, @tipo_gasto, @id_division, @division, @id_centro_costos ,@centro_costos , @id_usr_autoriza , @autorizador, @id_usr_autoriza2, @autorizador2, @id_usr_autorizador3, @autorizador3, @importe_anticipo, @importe_comprobado, @importe_devolucion, @vale_devolucion, @adj_importe_devolucion "
                    sdaSolicitud.SelectCommand = New SqlCommand(query, ConexionBD)
                    sdaSolicitud.SelectCommand.Parameters.AddWithValue("@id_ms_anticipo_proveedor", ._txtFolio.Text)
                    ...
                    sdaSolicitud.SelectCommand.Parameters.AddWithValue("@vale_devolucion", DBNull.Value)
                    ...
                    SCMValores.Parameters.Clear()
                    SCMValores.CommandText = "select max(id_ms_comprobacion_anticipo) from ms_comprobacion_anticipo where id_usr_solicita = @id_usr_solicita and estatus not in ('Z', 'A') "
                    ...
                    If ._txtBan.Text = 1 Then
                        ...
                        If ._txtEscenario.Text = "1" Then
                            ...
                            'Se insertan los datos de la factura CFDI EGRESO
                            SCMValores.CommandText = ""
                            SCMValores.Parameters.Clear()
                            SCMValores.CommandText = "EXEC SP_I_dt_comprobacion_anticipo @id_ms_comp_anticipo, @id_dt_factura, @fecha_emision, @uuid, @serie, @folio, @lugar_exp, @forma_pago, @moneda, @subtotal, @importe, @id_usr_carga"
                            SCMValores.Parameters.AddWithValue("@id_ms_comp_anticipo", .lblFolio.Text)
    ...
    Public Sub llenarFactura(ByVal tipo)
        With Me
            Try
                ...
                sdaFacturas.SelectCommand.Parameters.AddWithValue("@fecha", fecha)
        ...
        With Me
            Try
                ...
                sdaArchivos.SelectCommand = New SqlCommand("select nombre as archivo " +
                                                           ...
                                                           "where id_ms_comp_anticipo_proveedor = -1 and uuid_factura is null " +
...
#Region "Pago anticipado agente aduanal"
        ...
        With Me
            Try
                ...
                If .ddlProveedor.Items.Count = 0 Then
                    .litError.Text = "No existe el proveedor"
                End If
            Catch ex As Exception
                .litError.Text = ex.ToString
            End Try
    ...
    Public Function validarAdjuntos()
        With Me
            Try
                Dim ban As Integer = 0, uuid As String, val As Integer = 0
                Dim ConexionBD As SqlConnection = New System.Data.SqlClient.SqlConnection
                ConexionBD.ConnectionString = accessDB.conBD("ProcAd")
                Dim SCMValores As SqlCommand = New System.Data.SqlClient.SqlCommand
                SCMValores.Connection = ConexionBD
                ...
                For Each row As GridViewRow In gvAgenteAduanal.Rows
                    uuid = .gvAgenteAduanal.DataKeys(row.RowIndex).Values("uuid")
                    'idFactura = .gvAgenteAduanal.DataKeys(row.RowIndex).Value.ToString()


                    SCMValores.CommandText = ""
                    SCMValores.Parameters.Clear()
                    SCMValores.CommandText = " SELECT COUNT(*) FROM dt_archivo_adj_comp_anticipo WHERE uuid_factura = @uuid_factura"
                    SCMValores.Parameters.AddWithValue("@uuid_factura", uuid)
                    ConexionBD.Open()
                ...
                If ban > 0 Then
                    validarAdjuntos = False
                Else
                    validarAdjuntos = True
                End If
            Catch ex As Exception
                .litError.Text = ex.ToString()
                validarAdjuntos = False
    ...
    Public Function devolucion()
        With Me
            Try
                Dim importeComprobado As Double = 0, importeDevolucion As Double = 0

                'Se suman los importes de las facturas ingresadas

                For Each row As GridViewRow In gvAgenteAduanal.Rows
                    importeComprobado = importeComprobado + CDbl(row.Cells(9).Text.ToString())
                Next

                'Se calcula el importe a devolver
                importeDevolucion = Val(._txtImporte.Text) - importeComprobado
            ...
            Try
                If .ddlProveedor.Items.Count > 0 Then
                    ...
                    If .gvFacturasProveedor.Rows.Count() = 0 Then
                        .pnlFacturasProveedor.Visible = False
                        .litError.Text = "No existe registros para el proveedor seleccionado"
                    Else
                        .pnlFacturasProveedor.Visible = True
                        .ibtnAltaProveedor.Visible = True
                        .ibtnAltaProveedor.Enabled = False
                        .ibtnAltaProveedor.ImageUrl = "images\Add_i2.png"
                        .ibtnBajaProveedor.Visible = True
    ...
    Protected Sub gvFacturasProveedor_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvFacturasProveedor.SelectedIndexChanged
        With Me
            Try
                ...
                .ibtnAltaProveedor.ImageUrl = "images\Add.png"
            Catch ex As Exception
    ...
    Private Sub gvMsCargasCombustible_PageIndexChanging(sender As Object, e As GridViewPageEventArgs) Handles gvFacturasProveedor.PageIndexChanging
        gvFacturasProveedor.PageIndex = e.NewPageIndex
        llenargvFacturasProveedor("INGRESO")
    End Sub
    ...
    Public Sub llenargvAgenteAduanal()
        With Me
            Try
                ...
                If .gvAgenteAduanal.Rows.Count() > 0 Then
                    .pnlBtnAdjEvidenvias.Visible = True
                    .ibtnAlta.Enabled = False
                    .ibtnAlta.ImageUrl = "images\Add_i2.png"
                    .lbl_CFDIAnticipoAgenteAduanal.Visible = True
    ...
    Protected Sub gvAgenteAduanal_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvAgenteAduanal.SelectedIndexChanged
        With Me
            Try
                ...
                .ibtnBaja.Enabled = False
                .ibtnBaja.ImageUrl = "images\Trash_i2.png"
                .ibtnBajaProveedor.Enabled = True
                .ibtnBajaProveedor.ImageUrl = "images\Trash.png"

                .btnEvidenciaAgenteAduanal.Enabled = True
            Catch ex As Exception
    ...
    Protected Sub btnEvidenciaAgenteAduanal_Click(sender As Object, e As EventArgs) Handles btnEvidenciaAgenteAduanal.Click
        With Me
            Try
                ...
                'Verificar que el archivo ha sido seleccionado y es un archivo válido
                If (Not fuEvidenciaAgenteAduanal.PostedFile Is Nothing) And (fuEvidenciaAgenteAduanal.PostedFile.ContentLength > 0) Then
                    'Determinar el nombre original del archivo
                    Dim sFileName As String = System.IO.Path.GetFileName(fuEvidenciaAgenteAduanal.PostedFile.FileName)
                    Dim idArchivo As Integer 'Index correspondiente al archivo
                    Dim fecha As DateTime = Date.Now
                    Dim ConexionBD As SqlConnection = New System.Data.SqlClient.SqlConnection
                    ConexionBD.ConnectionString = accessDB.conBD("ProcAd")
                    ...
                    SCMValores.Connection = ConexionBD
                    Try
                        If fuEvidenciaAgenteAduanal.PostedFile.ContentLength <= lMaxFileSize Then

                            'Registrar el archivo en la base de datos
                            SCMValores.CommandText = ""
                            SCMValores.Parameters.Clear()
                            SCMValores.CommandText = "INSERT INTO dt_archivo_adj_comp_anticipo(id_ms_comp_anticipo_proveedor, id_usuario, nombre, fecha, uuid_factura ) values(-1, @id_usuario, @nombre, @fecha, @uuid_factura)"
                            SCMValores.Parameters.AddWithValue("@id_usuario", Val(Me._txtIdUsuario.Text))
                            SCMValores.Parameters.AddWithValue("@nombre", sFileName)
                            SCMValores.Parameters.AddWithValue("@fecha", fecha)
                            SCMValores.Parameters.AddWithValue("@uuid_factura", (.gvAgenteAduanal.DataKeys(gvAgenteAduanal.SelectedIndex).Values("uuid"))).ToString()
                            ConexionBD.Open()
                            SCMValores.ExecuteNonQuery()
                            ConexionBD.Close()
                            'Obtener el Id del archivo
                            ...
                            sFileName = idArchivo.ToString + "-" + sFileName
                            'Almacenar el archivo en la ruta especificada
                            fuEvidenciaAgenteAduanal.PostedFile.SaveAs(sFileDir + sFileName)
                            'lblMessage.Visible = True
                            'lblMessage.Text = "El archivo: " + sFileName + " se ha adjuntado exitosamente"
    ...
    Public Sub actualizarAdjuntosEsc5()
        With Me
            Try
                ...
                sdaArchivos.SelectCommand = New SqlCommand("select id_dt_archivo_adj_comp_anticipo, nombre as archivo" +
                                                           "     , 'http://148.223.153.43/ProcAd - Adjuntos CompAnt/' + cast(id_dt_archivo_adj_comp_anticipo as varchar(20)) + '-' + nombre as path, uuid_factura " +
                                                           "from dt_archivo_adj_comp_anticipo " +
                                                           "where id_ms_comp_anticipo_proveedor = -1 and uuid_factura is not null " +
                                                            "  and id_usuario = @idUsuario ", ConexionBD)
                sdaArchivos.SelectCommand.Parameters.AddWithValue("@idUsuario", Val(._txtIdUsuario.Text))
                ConexionBD.Open()
                sdaArchivos.Fill(dsArchivos)
                ...
                .upGvEvidencias.Visible = True

                If gvEvidencias.Rows.Count() > 0 Then
                    .btnAceptarAgenteAduanal.Visible = True
                End If
            Catch ex As Exception
                .litError.Text = ex.ToString
            End Try
    ...
    Protected Sub ibtnAlta_Click(sender As Object, e As ImageClickEventArgs) Handles ibtnAlta.Click
        With Me
            Try
                ...
                If .gvAgenteAduanal.SelectedIndex = -1 Then
                    .btnEvidenciaAgenteAduanal.Enabled = False
                Else
                    .btnEvidenciaAgenteAduanal.Enabled = True
                End If
            Catch ex As Exception
    ...
    Protected Sub ibtnBaja_Click(sender As Object, e As ImageClickEventArgs) Handles ibtnBaja.Click
        With Me
            Try
                ...
                If gvEvidencias.SelectedIndex = -1 Then
                    .litError.Text = "Seleccione el archivo a eliminar"
                Else
                    ' '' Ruta Local
                    'Dim sFileDir As String = "C:/ProcAd - Adjuntos CompAnt/" 'Ruta en que se almacenará el archivo
                    ' Ruta en Atenea
                    ...
                    sdaCatalogo.SelectCommand = New SqlCommand("DELETE FROM dt_archivo_adj_comp_anticipo WHERE id_dt_archivo_adj_comp_anticipo = @id_dt_archivo_adj_comp_anticipo", ConexionBD)
                    sdaCatalogo.SelectCommand.Parameters.AddWithValue("@id_dt_archivo_adj_comp_anticipo", (.gvEvidencias.DataKeys(gvEvidencias.SelectedIndex).Values("id_dt_archivo_adj_comp_anticipo")))
                    ConexionBD.Open()
            ...
            Try
                ...
                Else
                    ...
                    Else
                        ban = 1
                    End If
                    .litError.Text = .litError.Text + "Agregue por lo menos una evidencia por factura"
                End If
                ...
                If ban = 0 Then
                    ...
                    sdaSolicitud.SelectCommand.Parameters.AddWithValue("@importe_anticipo", Val(._txtImporte.Text))
                    sdaSolicitud.SelectCommand.Parameters.AddWithValue("@importe_comprobado", Val(._ImporteComprobado.Text))

                    If upValeIng.Visible = True Then
                        sdaSolicitud.SelectCommand.Parameters.AddWithValue("@importe_devolucion", Val(._ImporteDevolucion.Text))
                        sdaSolicitud.SelectCommand.Parameters.AddWithValue("@vale_devolucion", .txtValeIng.Text)
                    ...
                    SCMValores.Parameters.Clear()
                    SCMValores.CommandText = "select max(id_ms_comprobacion_anticipo) from ms_comprobacion_anticipo where id_usr_solicita = @id_usr_solicita and estatus = 'P'"
                    SCMValores.Parameters.AddWithValue("@id_usr_solicita", Val(._txtIdUsuario.Text))
                    ConexionBD.Open()
                    .lblFolio.Text = SCMValores.ExecuteScalar
                    ConexionBD.Close()
    ...
    Public Sub llenarAutorizador()
        With Me
            Try
                ...
                sdaAut.SelectCommand = New SqlCommand(" SELECT usr.id_usuario, cgEmpl.id_empleado, cgEmpl.nombre + ' ' + ap_paterno + ' ' + ap_materno AS autorizador " +
                                " FROM dt_autorizador " +
    ...
    Public Sub llenarAutorizador2()
        ...
        With Me
            Try
                ...
                sdaAut2.SelectCommand = New SqlCommand("select cg_usuario.id_usuario, cgEmpl.id_empleado, cgEmpl.nombre + ' ' + ap_paterno + ' ' + ap_materno as nombre_empleado " +
                                                              "from dt_autorizador " +

...
#Region "Funciones"
    ...
    Public Sub buscarEmpl()
        With Me
            ...
            sdaEmpleado.SelectCommand = New SqlCommand("select cg_usuario.id_usuario " + _
                                                       "     , cgEmpl.nombre + ' ' + ap_paterno + ' ' + ap_materno as nombre_empleado " + _
                                                       "from cg_usuario " + _
                                                       "  left join bd_Empleado.dbo.cg_empleado cgEmpl on cg_usuario.id_empleado = cgEmpl.id_empleado " + _
                                                       "where cgEmpl.nombre + ' ' + ap_paterno + ' ' + ap_materno like '%' + @empleado + '%' " + _
        ...
        With Me
            Try
                ...
                sdaEmpresa.SelectCommand = New SqlCommand("select Empresa.id_empresa, Empresa.nombre " + _
                                                          ...
                                                          "  and cg_usuario.id_usuario = @idUsrEmpleado " + _
                                                          "order by Empresa.nombre ", ConexionBD)
                sdaEmpresa.SelectCommand.Parameters.AddWithValue("@idUsrEmpleado", .ddlEmpleado.SelectedValue)
        ...
        With Me
            Try
                ...
                SCMValores.CommandText = "select isnull((select dtNoProv.no_proveedor " + _
                                         ...
                                         "	    		 where cg_usuario.id_usuario = @idUsrEmpleado " + _
                                         "				   and id_empresa = @idEmpresa), '') as no_proveedor "
                SCMValores.Parameters.AddWithValue("@idUsrEmpleado", .ddlEmpleado.SelectedValue)
            ...
            Try
                ...
                If .btnEnviar.Enabled = True Then
                    ...
                    While Val(._txtBan.Text) = 0
                        ...
                        sdaEmpleado.SelectCommand.Parameters.AddWithValue("@idAut", 47) 'ID Empleado de Carmen Medina
                        ConexionBD.Open()
                        sdaEmpleado.Fill(dsEmpleado)
                        ConexionBD.Close()
                        ...
                        'Insertar Anticipo
                        SCMValores.Parameters.Clear()
                        SCMValores.CommandText = "insert into ms_anticipo ( id_usr_solicita,  fecha_solicita,  id_usr_autoriza,  fecha_autoriza,  empresa,  no_empleado,  no_proveedor,  empleado,  no_autorizador,  autorizador,  destino,  actividad,  periodo_comp,  periodo_ini,  periodo_fin,  tipo_pago,  no_personas,  dias_hospedaje,  monto_hospedaje,  dias_alimentos,  monto_alimentos,  dias_casetas,  monto_casetas,  dias_otros,  monto_otros,  otros_especifico,  status,  tipo) " + _
                                                 " 			       values (@id_usr_solicita, @fecha_solicita, @id_usr_autoriza, @fecha_autoriza, @empresa, @no_empleado, @no_proveedor, @empleado, @no_autorizador, @autorizador, @destino, @actividad, @periodo_comp, @periodo_ini, @periodo_fin, @tipo_pago, @no_personas, @dias_hospedaje, @monto_hospedaje, @dias_alimentos, @monto_alimentos, @dias_casetas, @monto_casetas, @dias_otros, @monto_otros, @otros_especifico,     'A', 'AAE')"
                        SCMValores.Parameters.AddWithValue("@id_usr_solicita", .ddlEmpleado.SelectedValue)
                        SCMValores.Parameters.AddWithValue("@fecha_solicita", fecha)
                        SCMValores.Parameters.AddWithValue("@id_usr_autoriza", Val(._txtIdUsuario.Text))
                        ...
                        SCMValores.Parameters.AddWithValue("@id_actividad", 66)
                        ConexionBD.Open()
                        SCMValores.ExecuteNonQuery()
                        ConexionBD.Close()

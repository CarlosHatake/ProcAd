    ...
    Public Sub llenarGrid()
        With Me
            Try
                ...
                sdaCatalogo.SelectCommand = New SqlCommand("select id_concepto " +
                                                           ...
                                                           "  left join bd_Empleado.dbo.cg_cc on cg_concepto.id_cc = cg_cc.id_cc " +
                                                           "where cg_concepto.status = 'A' " +
        ...
        With Me
            Try
                ...
                sdaCatalogo.SelectCommand = New SqlCommand("select id_concepto_cat " +
                                                           ...
                                                           "from cg_concepto " +
                                                           "  left join bd_Empleado.dbo.cg_cc on cg_concepto.id_cc = cg_cc.id_cc " +
                                                           "where id_concepto = @id_concepto ", ConexionBD)
                sdaCatalogo.SelectCommand.Parameters.AddWithValue("@id_concepto", idConcepto)
                ConexionBD.Open()
                sdaCatalogo.Fill(dsCatalogo)
                ConexionBD.Close()
                ...
                .txtConcepto.Text = dsCatalogo.Tables(0).Rows(0).Item("nombre_concepto").ToString()
                .txtCuentaC.Text = dsCatalogo.Tables(0).Rows(0).Item("cuenta").ToString()
                .ddlTipoComp.SelectedValue = Val(dsCatalogo.Tables(0).Rows(0).Item("id_tipoComp").ToString())
                ...
                If dsCatalogo.Tables(0).Rows(0).Item("val_orig_destino").ToString() = "S" Then
                    .cbValOrigen_Destino.Checked = True
                Else
                    .cbValOrigen_Destino.Checked = False
                End If
                sdaCatalogo.Dispose()
                dsCatalogo.Dispose()
                bloqueoPantalla()
        ...
        With Me
            Try
                ...
                Select Case ._txtTipoMov.Text 'Tipo de Ajuste (alta, baja o modificación)
                    Case "A"
                        SCMTemp.CommandText = "SELECT COUNT(*) FROM cg_concepto WHERE nombre_concepto = @nombre_concepto AND abreviatura = @abreviatura AND id_cc = @id_cc "  '' AND status = 'A'
                    Case Else
                        SCMTemp.CommandText = "SELECT COUNT(*) FROM cg_concepto WHERE nombre_concepto = @nombre_concepto AND abreviatura = @abreviatura AND id_cc = @id_cc AND id_concepto <> @id_concepto"  '' AND status = 'A'
                        SCMTemp.Parameters.AddWithValue("@id_concepto", .gvConcepto.SelectedRow.Cells(0).Text)
                End Select
                SCMTemp.Parameters.AddWithValue("@nombre_concepto", .txtConcepto.Text)
            ...
            Try
                ...
                Else
                    ...
                    Select Case ._txtTipoMov.Text 'Tipo de Ajuste (alta, baja o modificación)
                        Case "A"
                            If validar() Then
                                SCMValores.CommandText = "INSERT INTO cg_concepto( id_concepto_cat,  abreviatura,  nombre_concepto,  cuenta,  id_tipoComp,  id_cc,  alimentos,  no_conceptos,  cve_concepto1,  cantidad1,  cve_concepto2,  cantidad2,  val_orig_destino) " +
                                                         "                 values(@id_concepto_cat, @abreviatura, @nombre_concepto, @cuenta, @id_tipoComp, @id_cc, @alimentos, @no_conceptos, @cve_concepto1, @cantidad1, @cve_concepto2, @cantidad2, @val_orig_destino)"
                            Else
                                .litError.Text = "Valor Inválido, ya existe ese Concepto"
                                ban = 1
                            End If
                        Case "B"
                            SCMValores.CommandText = "UPDATE cg_concepto SET status = 'B' WHERE id_concepto = @id_concepto"
                            SCMValores.Parameters.AddWithValue("@id_concepto", .gvConcepto.SelectedRow.Cells(0).Text)
                        Case Else
                            If validar() Then
                                SCMValores.CommandText = "UPDATE cg_concepto SET id_concepto_cat = @id_concepto_cat, abreviatura = @abreviatura, nombre_concepto = @nombre_concepto, cuenta = @cuenta, id_tipoComp = @id_tipoComp, id_cc = @id_cc, alimentos = @alimentos, no_conceptos = @no_conceptos, cve_concepto1 = @cve_concepto1, cantidad1 = @cantidad1, cve_concepto2 = @cve_concepto2, cantidad2 = @cantidad2, val_orig_destino = @val_orig_destino  WHERE id_concepto = @id_concepto"
                                SCMValores.Parameters.AddWithValue("@id_concepto", .gvConcepto.SelectedRow.Cells(0).Text)
                            Else
                                .litError.Text = "Valor Inválido, ya existe ese Concepto"
                                ban = 1
                    ...
                    If ban = 0 Then
                        ...
                        SCMValores.Parameters.AddWithValue("@nombre_concepto", .txtConcepto.Text)
                        SCMValores.Parameters.AddWithValue("@cuenta", .txtCuentaC.Text)
                        SCMValores.Parameters.AddWithValue("@id_tipoComp", .ddlTipoComp.SelectedValue)
                        ...
                        End If
                        If .cbValOrigen_Destino.Checked = True Then

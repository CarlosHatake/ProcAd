        ...
        With Me
            Try
                ...
                If Session("Error") = "" Then
                    ...
                    If _txtAutDir.Text = "S" Then

                        valor = 0
                        SCMValores.CommandText = ""
                        SCMValores.Parameters.Clear()
                        SCMValores.CommandText = "DECLARE @valorR int;  Execute Sp_U_solRecursos  @id_ms_recursos, @fechaAutoriza,  @id_actividad, @id_ms_instancia,  @id_usr,  @montoPgvEp, @valorR OUTPUT; select @valorR"
                        SCMValores.Parameters.AddWithValue("@fechaAutoriza", fecha)
                        SCMValores.Parameters.AddWithValue("@id_ms_recursos", Val(lblFolio.Text))
                        SCMValores.Parameters.AddWithValue("@id_actividad", 116)
                        ...
                        'SCMValores.Parameters.Clear()
                        'SCMValores.CommandText = "SP_U_ms_recursos @id_ms_recursos , @id_ms_instancia , @id_actividad , @id_usr "
                        'SCMValores.Parameters.AddWithValue("@id_ms_recursos", Val(.lblFolio.Text))
                        'SCMValores.Parameters.AddWithValue("@id_ms_instancia", Val(._txtIdMsInst.Text))
                        'SCMValores.Parameters.AddWithValue("@id_actividad", 116)
                        'SCMValores.Parameters.AddWithValue("@id_usr", Val(._txtIdUsuario.Text))
                    ...
                    Else
                        While Val(._txtBan.Text) = 0
                            ...
                            If (cblRecursos.Items(0).Selected = True And wceMontoH.Text <> "") Or cblRecursos.Items(3).Selected = True Then
                                ...
                                'SCMValores.CommandText = ""
                                'SCMValores.Parameters.Clear()
                                'SCMValores.CommandText = "update ms_recursos set fecha_autoriza = @fecha_autoriza where id_ms_recursos = @id_ms_recursos "
                            ...
                            Else
                                ...
                                'SCMValores.CommandText = ""
                                'SCMValores.Parameters.Clear()
                                'SCMValores.CommandText = "update ms_recursos set fecha_autoriza = @fecha_autoriza, status = 'A' where id_ms_recursos = @id_ms_recursos "
                                'SCMValores.Parameters.AddWithValue("@fecha_autoriza", fecha)
                                'SCMValores.Parameters.AddWithValue("@id_ms_recursos", Val(.lblFolio.Text))
                                'ConexionBD.Open()
                                'SCMValores.ExecuteNonQuery()
        ...
        With Me
            Try
                ...
                While Val(._txtBan.Text) = 0
                    ...
                    SCMValores.CommandText = ""
                    SCMValores.Parameters.Clear()
                    SCMValores.CommandText = "DECLARE @valorR int;  Execute Sp_U_solRecursos  @id_ms_recursos, @fechaAutoriza,  @id_actividad, @id_ms_instancia,  @id_usr,  @monto_pgv_ep , @valorR OUTPUT; select @valorR"
                    SCMValores.Parameters.AddWithValue("@fechaAutoriza", fecha)
                    If cblRecursos.Items(0).Selected = True Then
                    ...
                    'SCMValores.CommandText = ""
                    'SCMValores.Parameters.Clear()
                    'SCMValores.CommandText = "update ms_recursos set fecha_autoriza = @fecha_autoriza, monto_pgv_ep = @monto_pgv_ep, status = 'Z' where id_ms_recursos = @id_ms_recursos "
                    'SCMValores.Parameters.AddWithValue("@fecha_autoriza", fecha)
                    'If .cblRecursos.Items(0).Selected = True Then
                    '    SCMValores.Parameters.AddWithValue("@monto_pgv_ep", 0)
                    'Else

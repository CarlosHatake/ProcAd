    ...
    Public Sub actualizarUnidades()
        With Me
            Try
                ...
                sdaCatalogo.SelectCommand = New SqlCommand("select id_dt_unidad " + _
                                                           "     , empresa " + _
                                                           "     , no_economico " + _
                                                           "     , status " + _
                                                           "     , tipo " + _
    ...
    Protected Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        With Me
            Try
                ...
                query = "select id_ms_comp as no_comp " + _
                        "     , Empresa " + _
                        "     , empleado " + _
                        "     , autorizador " + _
                        "     , director " + _
                        "     , fecha_solicita " + _
                        "     , fecha_autoriza " + _
                        "     , case when autorizador = director then fecha_autoriza else fecha_aut_dir end as fecha_aut_dir " + _
                        "     , fecha_valida " + _
                        "     , periodo_ini " + _
                        "     , periodo_fin " + _
                        ...
                        "        where dt_anticipo.id_ms_comp = ms_comp.id_ms_comp) as importe_ant " + _
                        "     , (select sum(monto_total) as monto_comp " + _
                        "        from dt_comp " + _
                        "        where dt_comp.id_ms_comp = ms_comp.id_ms_comp) as importe_comp " + _
                        "     , importe_tot " + _
                        "     , case ms_comp.status " + _
                        ...
                        "       end as estatus " + _
                        "     , cg_actividad_inst.nombre_actividad " + _
                        "     , case pago_efectivo when 'S' then 'Sí' when 'N' then 'No' else null end as [Pago Efectivo] " + _
                        "     , fecha_efectivo as [Fecha Efectivo] " + _
                        "from ms_comp " + _
                        "  left join ms_instancia on ms_comp.id_ms_comp = ms_instancia.id_ms_sol and ms_instancia.tipo = 'C' " + _
                        "  left join cg_actividad_inst on ms_instancia.id_actividad = cg_actividad_inst.id_actividad " + _
                        "where empresa = 'EXPROMAT'  "
        ...
        With Me
            Try
                ...
                query = "select empresa " + _
                        ...
                        "     , justificacion " + _
                        "     , isnull(vale_ingreso, 'XX') as vale_ingreso " + _
                        "     , isnull(evidencia_adj, 'XX') as evidencia_adj " + _
                        "     , vale_ingreso_adj " + _
                        "     , aut_dir " + _
                        "     , director " + _
                        "     , (select count(*) from dt_unidad where dt_unidad.id_ms_comp = ms_comp.id_ms_comp) as unidades " + _
                        "from ms_comp " + _
                ...
                sdaAnticipo.SelectCommand = New SqlCommand("select ms_anticipo.id_ms_anticipo " + _
                                                           "     , periodo_ini " + _
                                                           "     , periodo_fin " + _
                ...
                sdaConcepto.SelectCommand = New SqlCommand("select fecha_realizo " + _
                                                           "     , case dt_comp.tipo when 'F' then 'F' else null end as Factura " + _
                                                           "     , case dt_comp.tipo when 'F' then 'http://148.223.153.43/Facturas/' + dt_factura.rfc_receptor + '/' + format(datepart(year, dt_factura.fecha_emision), '00') + '/' + upper(format(dt_factura.fecha_emision, 'MMMM', 'es-es')) + '/' + dt_factura.movimiento + '/' + dt_factura.tipo + '/' + dt_factura.uuid + '.pdf' else null end as path " + _
                                                           ...
                                                           "     , vehiculo " + _
                                                           "     , obs " + _
                                                           "     , dt_factura.id_dt_factura " + _
                                                           "from dt_comp " + _
                                                           "  left join dt_factura on dt_comp.CFDI = dt_factura.uuid and dt_factura.movimiento in ('RECIBIDAS', 'RECIBIDA') " + _
                                                           "where id_ms_comp = @id_ms_comp " + _
                ...
                query = "select (select isnull(sum(monto_hospedaje) + sum(monto_alimentos) + sum(monto_casetas) + sum(monto_otros), 0) as monto_ant " + _
                        "        from dt_anticipo " + _
                        "            left join ms_anticipo on dt_anticipo.id_ms_anticipo = ms_anticipo.id_ms_anticipo " + _
                        "        where dt_anticipo.id_ms_comp = @id_ms_comp) as anticipo_imp " + _
                        "     , (select sum(monto_total) as monto_comp " + _
                        "        from dt_comp " + _

                ...
                Try
                    ...
                    If Session("litError") = "" And Val(Session("id_usuario")) > 0 Then
                        ...
                        query = "select id_ms_negoc_servicio " + _
                                ...
                                "     , servicio " + _
                                "     , base " + _
                                "     , 'el: ' + convert(varchar, fecha_ini, 103) + ' al ' + convert(varchar, fecha_fin, 103) as vigencia " + _
                                "     , lugar_ejecucion " + _
                                "     , descripcion " + _
                                "     , isnull(comentario_aut_cotiza, '') as comentario_aut_cotiza " + _
                                "     , cFinanzas " + _
                                "     , cPresupuesto " + _
                                "     , isnull(comentario_cotiza, '') as comentario_cotiza " + _
                        ...
                        .txtComentarioVal.Text = dsSol.Tables(0).Rows(0).Item("comentario_aut_cotiza").ToString()
                        ...
                        'Nombre del Usuario que ingresa las cotizaciones
            ...
            Try
                ...
                If ban = 0 Then
                    ...
                    If (Not fuPDF1.PostedFile Is Nothing) And (fuPDF1.PostedFile.ContentLength > 0) And (Not fuPDF2.PostedFile Is Nothing) And (fuPDF2.PostedFile.ContentLength > 0) And (Not fuPDF3.PostedFile Is Nothing) And (fuPDF3.PostedFile.ContentLength > 0) Then
                        ' '' Ruta Local
                        ''Dim sFileDir As String = "C:/ProcAd - Adjuntos NegServ Cot/" 'Ruta en que se almacenará el archivo
                        ' Ruta en Atenea
                        Dim sFileDir As String = "D:\ProcAd - Adjuntos NegServ Cot\" 'Ruta en que se almacenará el archivo
                        ...
                        Try
                            ...
                            SCMValores.CommandText = ""
                            SCMValores.Parameters.Clear()
                            SCMValores.CommandText = "insert into dt_cotizacion_ns( id_ms_negoc_servicio,  no_cotizacion,  id_proveedor,  prov_codigo,  prov_nombre,  cantidad,  precio,  id_divisa,  tipo_cambio,  fecha_ini,  fecha_fin,  id_cond_pago,  nombre_archivo) " + _
                                                     "                      values(@id_ms_negoc_servicio, @no_cotizacion, @id_proveedor, @prov_codigo, @prov_nombre, @cantidad, @precio, @id_divisa, @tipo_cambio, @fecha_ini, @fecha_fin, @id_cond_pago, @nombre_archivo)"
                            ...
                            SCMValores.Parameters.Add("@cantidad", SqlDbType.VarChar)
                            ...
                            If fuPDF1.PostedFile.ContentLength <= lMaxFileSize Then
                                ...
                                Else
                                    ...
                                    ConexionBD.Close()
                                End If
                                SCMValores.Parameters("@cantidad").Value = .wneCantidad1.Value
                                ...
                                SCMValores.CommandText = "select max(id_dt_cotizacion_ns) from dt_cotizacion_ns where (id_ms_negoc_servicio = @id_ms_negoc_servicio) and (no_cotizacion = 1)"
                                ConexionBD.Open()
                            ...
                            If fuPDF2.PostedFile.ContentLength <= lMaxFileSize Then
                                ...
                                Else
                                    ...
                                    ConexionBD.Close()
                                End If
                                SCMValores.Parameters("@cantidad").Value = .wneCantidad2.Value
                                ...
                                ConexionBD.Close()
                                'Obtener el Id del archivo
                                SCMValores.CommandText = "select max(id_dt_cotizacion_ns) from dt_cotizacion_ns where (id_ms_negoc_servicio = @id_ms_negoc_servicio) and (no_cotizacion = 2)"
                                ConexionBD.Open()
                                idArchivo = SCMValores.ExecuteScalar
                                ConexionBD.Close()
                                'Se agrega el Id al nombre del archivo
                            ...
                            If fuPDF3.PostedFile.ContentLength <= lMaxFileSize Then
                                ...
                                Else
                                    ...
                                    ConexionBD.Close()
                                End If
                                SCMValores.Parameters("@cantidad").Value = .wneCantidad3.Value
                                ...
                                ConexionBD.Close()
                                'Obtener el Id del archivo
                                SCMValores.CommandText = "select max(id_dt_cotizacion_ns) from dt_cotizacion_ns where (id_ms_negoc_servicio = @id_ms_negoc_servicio) and (no_cotizacion = 3)"
                                ConexionBD.Open()
                                idArchivo = SCMValores.ExecuteScalar
                                ConexionBD.Close()
                                'Se agrega el Id al nombre del archivo
                            ...
                            SCMValores.CommandText = ""
                            SCMValores.Parameters.Clear()
                            SCMValores.CommandText = "update ms_negoc_servicio set fecha_ini = @fecha_ini, fecha_fin = @fecha_fin, id_usr_cotiza = @id_usr_cotiza, empleado_cotiza = @empleado_cotiza, fecha_cotiza = @fecha_cotiza, comentario_cotiza = @comentario_cotiza, status = 'CotI', proveedor_selec = @proveedor_selec, cotizacion_selec = @cotizacion_selec, cantidad = @cantidad, precio = @precio, id_divisa = @id_divisa, tipo_cambio = @tipo_cambio where id_ms_negoc_servicio = @id_ms_negoc_servicio "
                            SCMValores.Parameters.AddWithValue("@id_usr_cotiza", Val(._txtIdUsuario.Text))
                            SCMValores.Parameters.AddWithValue("@empleado_cotiza", ._txtNombreUsr.Text)
                            ...
                            Select Case .rblProvSel.SelectedValue
                                Case 1
                                    ...
                                    SCMValores.Parameters.AddWithValue("@cantidad", .wneCantidad1.Value)
                                    SCMValores.Parameters.AddWithValue("@precio", .wceImporte1.Value)
                                    SCMValores.Parameters.AddWithValue("@id_divisa", .ddlDivisa1.SelectedValue)
                                    SCMValores.Parameters.AddWithValue("@tipo_cambio", tipoCambio1)
                                    SCMValores.Parameters.AddWithValue("@fecha_ini", .wdpFechaIni1.Date)
                                    SCMValores.Parameters.AddWithValue("@fecha_fin", .wdpFechaFin1.Date)
                                Case 2
                                    SCMValores.Parameters.AddWithValue("@proveedor_selec", .ddlProveedor2.SelectedValue)
                                    SCMValores.Parameters.AddWithValue("@cantidad", .wneCantidad2.Value)
                                    SCMValores.Parameters.AddWithValue("@precio", .wceImporte2.Value)
                                    SCMValores.Parameters.AddWithValue("@id_divisa", .ddlDivisa2.SelectedValue)
                                    SCMValores.Parameters.AddWithValue("@tipo_cambio", tipoCambio2)
                                    SCMValores.Parameters.AddWithValue("@fecha_ini", .wdpFechaIni2.Date)
                                    SCMValores.Parameters.AddWithValue("@fecha_fin", .wdpFechaFin2.Date)
                                Case 3
                                    SCMValores.Parameters.AddWithValue("@proveedor_selec", .ddlProveedor3.SelectedValue)
                                    SCMValores.Parameters.AddWithValue("@cantidad", .wneCantidad3.Value)
                                    SCMValores.Parameters.AddWithValue("@precio", .wceImporte3.Value)
                                    SCMValores.Parameters.AddWithValue("@id_divisa", .ddlDivisa3.SelectedValue)
                                    SCMValores.Parameters.AddWithValue("@tipo_cambio", tipoCambio3)
                                    SCMValores.Parameters.AddWithValue("@fecha_ini", .wdpFechaIni3.Date)
                                    SCMValores.Parameters.AddWithValue("@fecha_fin", .wdpFechaFin3.Date)
                            End Select
                            ConexionBD.Open()
                            SCMValores.ExecuteNonQuery()
                            ConexionBD.Close()
                            ...
                            SCMValores.Parameters.AddWithValue("@id_actividad", 89)
                            ConexionBD.Open()
                            SCMValores.ExecuteNonQuery()
                            ConexionBD.Close()
                            ...
                            'Envío de Correo al Gerente de Compras
                            Dim Mensaje As New System.Net.Mail.MailMessage()
                            Dim destinatario As String = ""
                            'Obtener el Correos del Gerente de Compras
                            SCMValores.CommandText = "select valor from cg_parametros where parametro = 'ger_compras' "
                            ConexionBD.Open()
                            ...
                            Mensaje.Subject = "ProcAd - Solicitud de Negociación No. " + .lblFolio.Text + " por Autorizar"
                            Dim texto As String
                        ...
                        Catch exc As Exception    'En caso de error
                            ...
                            SCMValores.CommandText = ""
                            SCMValores.Parameters.Clear()
                            SCMValores.CommandText = "delete from dt_cotizacion_ns where id_ms_negoc_servicio = @id_ms_negoc_servicio"
                            SCMValores.Parameters.AddWithValue("@id_ms_negoc_servicio", Val(.lblFolio.Text))
                            ConexionBD.Open()
                            SCMValores.ExecuteNonQuery()
                            ConexionBD.Close()

            ...
            With Me
                Try
                    If Session("litError") = "" And Val(Session("id_usuario")) > 0 Then
                        ...
                        query = " SELECT id_ms_anticipo_proveedor, importe_requerido, empresa, justificacion, prov.rfc as RFCProveedor, prov.nombre as Proveedor FROM ms_anticipo_proveedor " +
    ...
    Protected Sub btnAutorizar_Click(sender As Object, e As EventArgs) Handles btnAutorizar.Click
        With Me
            Try
                ...
                SCMValores.CommandText = " DECLARE @valorR int; EXEC SP_U_ms_anticipo_proveedor_tesoreria @id_ms_anticipo_proveedor, @id_usr_pago_tesoreria, @fecha_reg_tesoreria, @fecha_prog_pago_tesoreria, @comentario_tesoreria, @id_ms_instancia, @id_actividad, @fecha, @id_usr, @valorR OUTPUT; select @valorR"
                SCMValores.Parameters.AddWithValue("@id_ms_anticipo_proveedor", .lblFolio.Text)
                SCMValores.Parameters.AddWithValue("@id_usr_pago_tesoreria", Val(._txtIdUsuario.Text))
                SCMValores.Parameters.AddWithValue("@fecha_reg_tesoreria", fecha)
                SCMValores.Parameters.AddWithValue("@fecha_prog_pago_tesoreria", .wdteFechaPagoAutorizar.Value)
                If .txtObservaciones.Text = "" Then
                    SCMValores.Parameters.AddWithValue("@comentario_tesoreria", DBNull.Value)
                Else
                    SCMValores.Parameters.AddWithValue("@comentario_tesoreria", .txtObservaciones.Text)
                End If
                SCMValores.Parameters.AddWithValue("@id_ms_instancia", Val(._txtIdMsInst.Text))
                SCMValores.Parameters.AddWithValue("@id_actividad", 138)
        ...
        With Me
            Try
                ...
                Else
                    ...
                    SCMValores.Parameters.AddWithValue("@id_ms_anticipo_proveedor", .lblFolio.Text)
                    SCMValores.Parameters.AddWithValue("@id_usr_pago_tesoreria", Val(._txtIdUsuario.Text))
                    SCMValores.Parameters.AddWithValue("@fecha_prog_pago_tesoreria", DBNull.Value)
                    SCMValores.Parameters.AddWithValue("@fecha_prog_pago_tesoreria", fecha)
                    SCMValores.Parameters.AddWithValue("@comentario_tesoreria", .txtObservaciones.Text)
                    SCMValores.Parameters.AddWithValue("@id_ms_instancia", Val(._txtIdMsInst.Text))
                    SCMValores.Parameters.AddWithValue("@id_actividad", 139)
                    SCMValores.Parameters.AddWithValue("@fecha", fecha)
                    SCMValores.Parameters.AddWithValue("@id_usr", Val(._txtIdUsuario.Text))
                    ConexionBD.Open()
                    valor = SCMValores.ExecuteScalar()

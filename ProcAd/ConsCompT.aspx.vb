            ...
            With Me
                Try
                    ...
                    If Session("litError") = "" And Val(Session("id_usuario")) > 0 Then
                        ...
                        sdaEmpleado.SelectCommand = New SqlCommand("select transporte, perfil, nombre + ' ' + ap_paterno + ' ' + ap_materno as empleado " + _
                                                                   "from cg_usuario " + _
                        ...
                        ._txtTransporte.Text = dsEmpleado.Tables(0).Rows(0).Item("transporte").ToString()
                        ...
                        query = "select distinct(ms_comp.Empleado) as solicito " + _
                                "from ms_comp " + _
                                "  left join bd_empleado.dbo.cg_empresa on ms_comp.empresa = cg_empresa.nombre " + _
                                "  left join bd_empleado.dbo.cg_cc on cg_empresa.id_empresa = cg_cc.id_empresa and cg_cc.nombre = ms_comp.centro_costo " + _
                                "  left join bd_empleado.dbo.cg_div on cg_empresa.id_empresa = cg_div.id_empresa and cg_div.nombre = ms_comp.division " + _
                                "where ms_comp.status in ('R') " + _
                                "  and case when (cg_cc.transporte is null) then cg_div.transporte else cg_cc.transporte end in (1, @transporte) " + _
                                "order by solicito "
                        sdaSolicitante.SelectCommand = New SqlCommand(query, ConexionBD)
                        sdaSolicitante.SelectCommand.Parameters.AddWithValue("@transporte", Val(._txtTransporte.Text))
                        .ddlSolicitante.DataSource = dsSolicitante
                        .ddlSolicitante.DataTextField = "solicito"
                        .ddlSolicitante.DataValueField = "solicito"
                        ConexionBD.Open()
                        ...
                        sdaEmpresa.SelectCommand.Parameters.AddWithValue("@transporte", Val(._txtTransporte.Text))
                        ...
                        sdaAutorizador.SelectCommand.Parameters.AddWithValue("@transporte", Val(._txtTransporte.Text))
        ...
        With Me
            Try
                ...
                queryF = "select ms_comp.no_empleado as [Id Empleado] " +
                         ...
                         "  left join bd_empleado.dbo.cg_puesto on cg_empleado.id_puesto = cg_puesto.id_puesto " +
                         "  left join bd_empleado.dbo.cg_empresa on ms_comp.empresa = cg_empresa.nombre " +
                         "  left join bd_empleado.dbo.cg_cc on cg_empresa.id_empresa = cg_cc.id_empresa and cg_cc.nombre = ms_comp.centro_costo " +
                         "  left join bd_empleado.dbo.cg_div on cg_empresa.id_empresa = cg_div.id_empresa and cg_div.nombre = ms_comp.division " +
                         "where dt_comp.tipo = 'F' " +
                         "  and ms_comp.status in ('R') " +
                         "  and case when (cg_cc.transporte is null) then cg_div.transporte else cg_cc.transporte end in (1, @transporte) "

                queryT = "select ms_comp.no_empleado as [Id Empleado] " +
                         "     , ms_comp.Empleado " +
                         "     , ms_comp.Empresa " +
                         "     , case when (centro_costo is null) then division else centro_costo end as [CC / División] " +
                         "     , cg_puesto.nombre as puesto " +
                ...
                If .cbSolicitante.Checked = True Then
                    queryF = queryF + "  and ms_comp.Empleado = @empleado "
                    queryT = queryT + "  and ms_comp.Empleado = @empleado "
                End If
                ...
                sdaConsulta.SelectCommand = New SqlCommand(queryF + _
                                                           "union all " + _
                                                           queryT + _
                                                           "order by empleado, Tipo, fecha_realizo, Empresa, [CC / División] ", ConexionBD)

                sdaConsulta.SelectCommand.Parameters.AddWithValue("@transporte", Val(._txtTransporte.Text))
                If .cbSolicitante.Checked = True Then

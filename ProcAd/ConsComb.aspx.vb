...
#Region "Funciones"
    ...
    Public Sub limpiar()
        With Me
            ...
            .cbNoComb.Checked = False
            .pnlNoComb.Visible = False
...
#Region "Filtros"
    ...
    Protected Sub cbNoSol_CheckedChanged(sender As Object, e As EventArgs) Handles cbNoComb.CheckedChanged
        vista(Me.pnlNoComb, Me.cbNoComb.Checked)
        If Me.cbNoComb.Checked = True Then
            Me.txtNoComb.Text = ""
        End If
    End Sub
        ...
        With Me
            Try
                ...
                query = "select id_ms_comb " + _
                        ...
                        "         when 'D' then 'Dispersado' " + _
                        "         when 'Z' then 'No Autorizado' " + _
                        "         when 'ZC' then 'Rechazado por Comprobaciones' " + _
                        "         else '-' " + _
                        "       end as estado " + _
                        "from ms_comb " + _
                        "where 1 =1  "
                ...
                End If
                If .cbNoComb.Checked = True Then
                    query = query + "  and id_ms_comb = @id_ms_comb "
                End If
                If .cbStatus.Checked = True Then
                    query = query + "  and status in (" + .ddlStatus.SelectedValue + ") "
                End If
                If .cbNoEco.Checked = True Then
                    query = query + "  and ms_comb.no_eco like '%' + @no_eco + '%' "
                End If
                query = query + "order by id_ms_comb "
                ...
                End If
                If .cbNoComb.Checked = True Then
                    sdaConsulta.SelectCommand.Parameters.AddWithValue("@id_ms_comb", .txtNoComb.Text.Trim)
                End If
                If .cbNoEco.Checked = True Then
